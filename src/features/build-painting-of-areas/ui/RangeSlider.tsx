...
interface Props {
	control: Control<RangeFormValues>;
...
export const RangeSlider: FC<Props> = ({
	...
	return (
		...
		>
				...
				{fields.map((field, i) => {
					...
					return (
						<div
							...
							className='absolute top-1/2 -translate-x-1/2 -translate-y-1/2 w-3.5 h-3.5 sm:w-7 sm:h-7
					...
					return (
						<div key={field.id} className='flex items-center gap-1'>
							...
							<input
								value={r.min}
								readOnly
								className='w-12 border border-border-gray rounded pl-2  bg-gray-100'
							/>
							...
							<input
								...
								className='w-12 border border-border-gray rounded pl-2 '
							/>
							...
							<button
								...
								className='px-2 py-1 rounded bg-text-red text-white text-sm'
							>
								✕
							</button>
						</div>
			...
			<Button variant='transparent' onClick={addRange} className='mt-4'>
				Добавить
			</Button>
		</div>
	);
